...
define ['underscore', '../utils/promises', '../utils/version'], (_, promises, version)->
  (apiFn, config, emitter, authServices=[]) ->
    ...
    login = (params, options={}, callback) ->
      ...
      if _.isString(params)
        if _.isString(options)
          ...
          params = { login:params, password:options }
        else
          # Hull.login('facebook','opts')
          params = { provider: params }

      # Detect if we're on Mobile. If so, do a Redirect login by default.
      # And setup 'display' to be 'touch' for Facebook Login
      #http://stackoverflow.com/questions/11381673/javascript-solution-to-detect-mobile-browser
      if isMobile()
        # We're on mobile, setup defaults for Touch/Redirect login
        options.display ||= 'touch'
        params.strategy ||= 'redirect'
      else
        # We're on Desktop, setup defaults for Popup/Popup login for Facebook
        if params.provider == 'facebook'
          options.display ||= if params.strategy=='redirect' then 'page' else 'popup'

      # Set defaults for Redirect to current page if redirecting.
      if params.strategy == 'redirect'
      ...
      if params.provider?
        ...
        promise = loginWithProvider(params,options).then(refresh)
        ...
        throw new Error('Seems like something is wrong in your Hull.login() call, We need a login and password fields to login. Read up here: http://www.hull.io/docs/references/hull_js/#user-signup-and-login') unless params.login? and params.password?

        # TODO : Do we ignore `options` in this email login scenario ?

        # Early return since we're leaving the page.
        return postForm(config.orgUrl+'/api/v1/users/login', 'post', params) if params.strategy=='redirect'

        promise = apiFn('users/login', 'post', _.pick(params, 'login', 'password')).then(refresh)
    ...
    loginWithProvider = (params, options)->
      return module.isAuthenticating() if module.isAuthenticating()

      authenticating = promises.deferred()
      unless ~(_.indexOf(authServices, params.provider ))
        authenticating.reject
          message: "No authentication service #{params.provider} configured for the app"
          reason: 'no_such_service'
        return authenticating.promise

      authenticating.provider = params.provider

      authUrl = module.authUrl(config, params, options)
      ...
      if params.strategy == 'redirect'
        ...
        module.authHelper(authUrl, params, options)
    ...
    generateAuthUrl = (config, params={}, options={})->
      module.createCallback()

      options.app_id        = config.appId
      # The following is here for backward compatibility. Must be removed at first sight next time
      options.callback_url  = params.redirect_url || options.callback_url || config.callback_url || config.callbackUrl || module.location.toString()
      options.auth_referer  = module.location.toString()
      options.version       = version
      querystring = _.map options,(v,k) ->
        encodeURIComponent(k)+'='+encodeURIComponent(v)
      .join('&')
      "#{config.orgUrl}/auth/#{params.provider}?#{querystring}"
    ...
    module =
      ...
      createCallback: ->
        window.__hull_login_status__ = (hash) ->
          ...
          onCompleteAuthentication(hash)
      authHelper: (path, params={}, options={})->

        openerString = "location=0,status=0"

        # Handle smaller Facebook popup windows
        if params.provider == 'facebook' and params.strategy='popup'
          width = 500
          height = 400
        else
          width = 1030
