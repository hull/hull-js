define(['sandbox'], function(sandbox) {
  return sandbox.widgets.create({
    ...
    datasources: {
      'facebookFriends' : sandbox.data.api('facebook/me/friends')
    },

    initialize: function() {
      this.quiz = this.data.api.model(this.id);
      this.pubsub.on('hull.model.' + this.id + '.change', function() { this.render() }.bind(this));
    ...
    actions: {
      ...
      login: function(source, e, options) {
        this.login(options.provider, options).then(this.startQuiz.bind(this))
      },
      ...
      submit: function() {
        ...
        var res  = this.data.api.post("hull", this.id + "/achieve", {
    ...
    reset: function() {
      ...
      this.answers = {};
      this.currentQuestionIndex = 0;
      this.currentUserId = this.data.api.model('me').id;
    },
